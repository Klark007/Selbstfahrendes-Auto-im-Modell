...
timeForward1m = 7.4 # 5.7                     # Number of seconds needed to move about 1 meter
timeBackward1m = 11.45 
timeSpinLeft = 6.45 # 4.8                     # Number of seconds needed to make a full left / right spin
timeSpinRight = 6.6
...
def PerformSpin(angle):
    if angle < 0.0:
        ...
        numSeconds = (angle / 360.0) * timeSpinLeft
    else:
        # Right turn
        driveLeft  = +1.0
        driveRight = -1.0
        # Calculate the required time delay
        numSeconds = (angle / 360.0) * timeSpinRight
...
if testMode:
    ...
    # Check distance
    input('Check distance, Press ENTER to start')
    ...
    PerformDrive(-0.3)
    # Check spinning
    input('Check spinning, Press ENTER to continue')
    print('Spinning left')
    PerformSpin(-360)
    input('Press ENTER to continue')
    ...
    sys.exit(0)
    """
    input('Going forwards. Please stop after 1 meter')
    start_time = time.time()
    PerformDrive(+1)
    input('Please press enter after 1 meter')
    ZB.MotorsOff()
    print('Time for +1 meter:', str(time.time() - start_time))
    
    input('Going backwards. Please stop after 1 meter')
    start_time = time.time()
    PerformDrive(-1)
    input('Please press enter after 1 meter')
    ZB.MotorsOff()
    ...
    input('Spining right. Please press enter after 360 degree')
    start_time = time.time()
    PerformSpin(+360)
    input('Please press enter after 360 degree')
    ZB.MotorsOff()
    print('Time for +360 degree:', str(time.time() - start_time))
    
    input('Spining left. Please press enter after 360 degree')
    start_time = time.time()
    PerformSpin(-360)
    input('Please press enter after 360 degree')
    ZB.MotorsOff()
